...
const Blog = () => {
  ...
  const toggleLike = async () => {
    ...
    setLikeLoading(true)
    try {
      const response = await axios.post(`/api/blog/toggle-like`, { blogId: id })
    ...
    <div className='relative'>
      ...
      <div className='text-center mt-20 text-gray-600'>
        ...
        <div className="flex items-center justify-center gap-6 mb-6">
          ...
          >
            <svg 
              className={`w-5 h-5 ${data.isLiked ? 'fill-red-500 text-red-500' : 'text-gray-400'}`} 
            ...
            <span>{likeLoading ? '...' : (data.likeCount || 0)}</span>
            <span className="text-sm">{data.isLiked ? 'Liked' : 'Like'}</span>
          </button>
        </div>
      </div>
          ...
          <img 
            src={
              data.image.startsWith('http') 
                ? data.image 
                : `${import.meta.env.VITE_BASE_URL || 'http://localhost:3000'}${data.image.startsWith('/') ? data.image : '/' + data.image}`
            }
            alt="" 
